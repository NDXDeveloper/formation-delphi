...
## Avant l'opérateur ternaire : la fonction `IfThen`

Avant Delphi 13, on simulait l'opérateur ternaire avec la fonction `IfThen` (présente dans `System.Math` pour les types numériques et dans `System.StrUtils` pour les chaînes) :
...
var
  ...
  Statut: string;
begin
  Max := IfThen(X > Y, X, Y);                   // IfThen numérique
  Statut := IfThen(Age >= 18, 'Majeur', 'Mineur'); // IfThen pour chaînes
end;
```

> ⚠️ **Différence avec l'opérateur ternaire** : `IfThen` est une fonction normale, donc ses **deux arguments sont toujours évalués** (pas de court-circuit). Si l'évaluation d'un argument peut planter ou avoir des effets de bord, préférez l'opérateur ternaire de Delphi 13 (`if … then … else …`), qui n'évalue que la branche sélectionnée :
>
> ```pascal
> // Avec IfThen : Liste.First est toujours évalué, plante si Liste est vide !
> Premier := IfThen(Liste.Count > 0, Liste.First, 0);
>
> // Avec l'opérateur ternaire : Liste.First n'est évalué que si Liste.Count > 0
> Premier := if Liste.Count > 0 then Liste.First else 0;
> ```
