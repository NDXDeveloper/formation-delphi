...
- `private` : section pour les membres privés (accessibles à l'intérieur de la classe et du code de la même unité — voir la section *Sections de visibilité* plus bas)
...
begin
  ...
  try
    // Utiliser l'objet
    UnePersonne.FNom := 'Marie';    // accessible dans la même unité (voir note)
    UnePersonne.FAge := 25;
...
> ⚠️ **Important** : Les champs `FNom` et `FAge` sont déclarés `private`. En Object Pascal, `private` autorise l'accès depuis **la même unité** où la classe est définie. Si vous voulez bloquer l'accès **même au sein de la même unité**, utilisez `strict private` (recommandé pour une encapsulation forte). En pratique, on accède aux champs privés depuis l'extérieur via des **propriétés** (voir section 3.7.2).

### Points importants

1. **Create** : méthode héritée qui alloue l'instance en mémoire et exécute le constructeur
2. **try...finally** : bloc garantissant la libération de la mémoire
3. **Free** : méthode qui vérifie d'abord que l'objet est non `nil`, puis appelle son destructeur `Destroy`. C'est pourquoi `Free` est sûr même si la variable vaut `nil`, contrairement à un appel direct `Destroy` qui planterait.
...
> ⚠️ **Note pédagogique** : L'accès direct à `FTitulaire` et `FSolde` ne fonctionne que parce que ce code est écrit dans la **même unité** que `TCompteBancaire` (rappel : en Object Pascal, `private` autorise l'accès depuis la même unité). Dans une vraie application, on exposerait ces champs via des **propriétés** (voir section 3.7.2), et un constructeur prendrait les valeurs initiales en paramètres. Cet exemple est volontairement simplifié pour illustrer la déclaration d'une classe.
...
### Private
Les membres `private` sont accessibles depuis la classe et **depuis tout le code écrit dans la même unité**. C'est une particularité historique d'Object Pascal.

```pascal
type
  TMaClasse = class
  private
    FDonneeSecrete: string;  // Accessible dans TMaClasse + dans la même unité
  end;
```

### Strict private
Les membres `strict private` sont accessibles **uniquement depuis la classe elle-même**, même pas depuis du code situé dans la même unité. C'est la visibilité la plus restrictive et la plus proche du `private` d'autres langages (Java, C#).

```pascal
type
  TMaClasse = class
  strict private
    FCompteur: Integer;  // Accessible UNIQUEMENT dans TMaClasse
  end;
```
...
### Protected
Les membres `protected` sont accessibles dans la classe, dans les classes dérivées, et dans **le code de la même unité** (comme `private`).

```pascal
type
  TMaClasse = class
  protected
    FDonneeProtegee: string;  // Accessible dans cette classe, ses descendants, et la même unité
  end;
```

### Strict protected
Comme `protected`, mais sans la « porte ouverte » de la même unité : accessible uniquement à la classe et à ses descendants, où qu'ils soient.

### Published
Les membres `published` sont publics et visibles dans l'inspecteur d'objets de Delphi (pour les propriétés). Ils sont en plus accessibles via la RTTI (Runtime Type Information), ce qui permet aux outils comme le concepteur de formulaires de les manipuler par réflexion.

> 💡 **Récapitulatif** : pour une encapsulation rigoureuse comparable au `private` de Java/C#, utilisez **`strict private`**. Réservez `private` aux cas où vous souhaitez expressément permettre aux autres classes de la même unité d'accéder aux champs (cas fréquent dans la VCL elle-même).
