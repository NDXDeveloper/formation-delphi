...
Une **propriété indexée** s'utilise avec des crochets `[ ]` comme un tableau. Elle est idéale pour exposer une collection interne.

```pascal
type
  TRepertoire = class
  private
    FNoms: array of string;
    function GetNom(Index: Integer): string;
    procedure SetNom(Index: Integer; const Value: string);
    function GetCount: Integer;
  public
    constructor Create(ATaille: Integer);
    property Noms[Index: Integer]: string read GetNom write SetNom; default;
    property Count: Integer read GetCount;
  end;

constructor TRepertoire.Create(ATaille: Integer);
begin
  inherited Create;
  SetLength(FNoms, ATaille);
end;

function TRepertoire.GetNom(Index: Integer): string;
begin
  Result := FNoms[Index];
end;

procedure TRepertoire.SetNom(Index: Integer; const Value: string);
begin
  FNoms[Index] := Value;
end;

function TRepertoire.GetCount: Integer;
begin
  Result := Length(FNoms);
end;
...
begin
  Repertoire := TRepertoire.Create(3);
  try
    Repertoire.Noms[0] := 'Alice';     // Accès via le nom de la propriété
    Repertoire[1] := 'Bob';            // Accès direct (grâce à `default`)
    Repertoire[2] := 'Charlie';

    ShowMessage(Repertoire[0]);        // 'Alice'
  finally
    Repertoire.Free;
  end;
end;
...
> 💡 **Le mot-clé `default`** : Quand une propriété indexée est marquée `default`, on peut omettre son nom et utiliser directement la notation `Objet[Index]`. C'est ce qui permet d'écrire `MaListe[3]` plutôt que `MaListe.Items[3]` pour les classes comme `TStringList` ou `TList<T>`.
...
begin  
  // Pour simplifier l'exemple, on compare directement. Voir la note ci-dessous.
  Result := (FLargeur = FHauteur);
end;
...
> ⚠️ **Comparaison de réels (`=`)** : Comparer deux nombres `Double` avec `=` est généralement à éviter car les calculs en virgule flottante peuvent générer de minuscules erreurs d'arrondi (`0.1 + 0.2 ≠ 0.3` exactement). Pour des comparaisons robustes, utilisez `SameValue(A, B)` ou `SameValue(A, B, Epsilon)` de l'unité `System.Math` :
>
> ```pascal
> uses System.Math;
> Result := SameValue(FLargeur, FHauteur);
> ```
