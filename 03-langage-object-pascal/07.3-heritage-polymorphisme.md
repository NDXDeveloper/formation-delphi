...
Le polymorphisme brille vraiment quand on traite des collections (nécessite l'unité `System.Generics.Collections` dans la clause `uses`) :
...
> 💡 **Astuce** : Si vous voulez que la libération des éléments soit prise en charge automatiquement, utilisez `TObjectList<TAnimal>.Create(True)`. Tous les objets ajoutés seront détruits lors de la libération de la liste, ce qui évite la boucle `for ... do Animal.Free`.
...
> ⚠️ **Attention** : Une classe contenant au moins une méthode `abstract` ne devrait jamais être instanciée directement. Si vous appelez `TForme.Create` puis `Forme.Dessiner`, le compilateur émet l'avertissement `W1020 Constructing instance of '%s' containing abstract method '%s'` et l'exécution lèvera une `EAbstractError`. Pour empêcher cette instanciation dès la compilation, vous pouvez marquer la classe elle-même comme `abstract` :
>
> ```pascal
...
**Attention** : `reintroduce` brise le polymorphisme. Préférez toujours `override` quand c'est possible. En pratique, `reintroduce` sert surtout à supprimer le **warning de compilation** quand on définit volontairement une méthode avec la même signature qu'une méthode héritée non virtuelle (ou avec une signature différente d'une méthode héritée virtuelle).

### virtual vs dynamic

En plus de `virtual`, Object Pascal propose le mot-clé `dynamic` qui marque aussi une méthode comme redéfinissable :

```pascal
type
  TBase = class
    procedure MaMethode; dynamic;
  end;
```

La **différence** entre `virtual` et `dynamic` est uniquement interne : `virtual` utilise une *VMT* (Virtual Method Table) plus rapide à dispatcher mais plus volumineuse, tandis que `dynamic` utilise une *DMT* (Dynamic Method Table) plus compacte mais légèrement plus lente. La sémantique vis-à-vis de `override` est identique.

> 💡 En pratique, **utilisez toujours `virtual`** sauf cas exceptionnel d'optimisation mémoire (héritage très profond, méthodes rarement appelées). C'est la convention dans toute la VCL.
...
Vous pouvez aussi sceller une **classe entière** pour interdire l'héritage. Cela est utile pour les classes finales qui ne doivent jamais être étendues :

```pascal
type
  TMaClasseFinale = class sealed
    // ...
  end;

  // Erreur de compilation : impossible d'hériter d'une classe sealed
  TDescendant = class(TMaClasseFinale)
  end;
```
