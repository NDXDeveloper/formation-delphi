...
Si une exception est levée dans un constructeur, l'objet est considéré comme partiellement construit. Bonne nouvelle : Delphi appelle **automatiquement** le destructeur `Destroy` dans ce cas, à condition que celui-ci soit correctement écrit pour tolérer des champs non initialisés.

Cela fonctionne grâce à deux garanties du langage :
- Les champs d'instance sont **automatiquement initialisés à zéro / nil** par `TObject.Create` avant l'exécution de votre constructeur.
- La méthode `Free` est sûre lorsqu'elle est appelée sur un pointeur `nil` (elle ne fait rien).
...
begin  
  ...
  // Si TFileStream.Create lève une exception, Delphi appelle automatiquement
  // TRessource.Destroy : FDonnees sera libéré, FFichier vaut nil donc Free ne
  // fait rien sur lui.
end;

destructor TRessource.Destroy;  
begin  
  FFichier.Free;   // Sûr même si FFichier est nil
  FDonnees.Free;   // Sûr même si FDonnees est nil
  inherited Destroy;
end;
```

> 💡 **À retenir** : Écrivez un destructeur **robuste face aux champs nil**, et vous n'aurez plus besoin de blocs `try/except` défensifs dans vos constructeurs. C'est la pratique idiomatique en Object Pascal.
...
### 2. Initialiser explicitement les champs aux valeurs non « zéro »

En Object Pascal, **tous les champs d'instance sont automatiquement mis à zéro / `''` / `nil` / `False`** par `TObject.Create`. Vous n'avez donc pas besoin de répéter ces valeurs par défaut.

En revanche, initialisez explicitement tout champ qui doit avoir une **valeur non triviale** :

```pascal
constructor TPersonne.Create;  
begin  
  inherited Create;
  // FNom, FAge, FActif déjà à '', 0, False — pas besoin d'écrire ça
  FDateCreation := Now;    // ← valeur non triviale, à initialiser
  FStatut := psNouveau;    // ← énumération autre que la première valeur
end;
```

> 💡 Cette mise à zéro automatique fonctionne aussi pour vos propres types : les chaînes deviennent `''`, les tableaux dynamiques `nil`, les interfaces `nil`. C'est ce qui rend la « robustesse face aux champs nil » de la section précédente sûre.
