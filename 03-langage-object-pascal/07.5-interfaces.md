...
2. **GUID** : un identifiant unique global entre crochets. Techniquement optionnel, mais **fortement recommandé** car il est requis pour :
   - L'opérateur `as` (cast d'objet vers interface)
   - La fonction `Supports` (test d'implémentation)
   - L'introspection via la RTTI
...
`TInterfacedObject` (déclarée dans `System`) est une classe de base qui implémente déjà l'interface racine `IInterface` (anciennement `IUnknown`). Concrètement, elle fournit les trois méthodes obligatoires de toute interface :

- `QueryInterface` : permet de demander à l'objet s'il implémente une autre interface (utilisée par `Supports` et `as`).
- `_AddRef` : incrémente le compteur de références.
- `_Release` : décrémente le compteur ; lorsque celui-ci atteint zéro, l'objet se détruit automatiquement.

C'est ce mécanisme qui rend la gestion mémoire automatique : tant qu'au moins une variable d'interface référence l'objet, le compteur reste ≥ 1, l'objet reste en vie. Dès que la dernière référence sort de portée, l'objet est libéré.

> 💡 Vous pouvez bien sûr écrire vos propres classes implémentant `IInterface` à la main, mais `TInterfacedObject` couvre 95 % des cas réels et c'est l'idiome standard.
...
begin
  // Une seule création, ensuite tout passe par les interfaces.
  // Le comptage de références se charge de la libération.
  Dessinable := TRectangle.Create(10, 5);
  Dessinable.Dessiner;

  // Récupération d'une autre vue de l'objet via les interfaces
  if Supports(Dessinable, IRedimensionnable, Redimensionnable) then
    Redimensionnable.Agrandir(2);

  if Supports(Dessinable, IColorable, Colorable) then
...
> ⚠️ **Piège à éviter** : Ne mélangez pas la gestion d'interface (comptage de références) et la gestion d'objet (`Free` manuel) sur le même `TInterfacedObject`. Une fois qu'une référence d'interface pointe vers l'objet, c'est elle qui décidera de sa destruction. Si vous appelez `MonRectangle.Free` alors qu'une variable d'interface pointe toujours sur l'objet, vous risquez un crash (`Invalid pointer operation` ou `EAccessViolation`).

## L'opérateur Supports

L'opérateur `Supports` (unité `System.SysUtils`) vérifie si un objet implémente une interface donnée. Il est particulièrement utile quand vous travaillez avec une référence d'interface dont vous ignorez le type concret :
...
var
  ...
  Redimensionnable: IRedimensionnable;
begin
  Forme := TRectangle.Create(10, 5);  // Tout passe par l'interface

  // L'objet implémente-t-il une autre interface ?
  if Supports(Forme, IRedimensionnable, Redimensionnable) then
  begin
    ShowMessage('La forme est redimensionnable');
    Redimensionnable.Agrandir(2);
  end
  else
    ShowMessage('La forme n''est pas redimensionnable');

  // Libération automatique en fin de bloc
end;
```

> ⚠️ **Avec un `TInterfacedObject`, n'utilisez pas `Supports` sur une variable `TObject` que vous comptez libérer manuellement avec `Free`** : dès que `Supports` assigne le paramètre de sortie, le compteur de références passe à 1 et un futur `Free` provoque la même collision que celle décrite plus haut.

## L'opérateur as avec les interfaces

On peut aussi utiliser `as` pour convertir vers une interface (lève une exception si la conversion échoue) :

```pascal
var
  Dessinable: IDessinable;
begin
  // L'objet est conservé en vie tant que Dessinable existe.
  Dessinable := TRectangle.Create(10, 5) as IDessinable;
  Dessinable.Dessiner;
  // Libération automatique en fin de bloc.
end;
```
