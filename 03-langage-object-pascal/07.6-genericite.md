...
var
  Personne: TPersonne;
begin
  // `Creer` est une méthode de classe : on l'appelle directement sur le type
  // sans avoir besoin d'instancier TFactory.
...
begin
  Comparer := TComparer<T>.Default;  // Nécessite System.Generics.Defaults
...
begin
  ...
  try
    ...
    // Accéder à une valeur (lève EListError si la clé n'existe pas)
    ShowMessage('Âge de Bob : ' + IntToStr(Ages['Bob']));

    // Accès sûr avec TryGetValue (recommandé si la clé peut être absente)
    if Ages.TryGetValue('David', Age) then
      ShowMessage('Âge de David : ' + IntToStr(Age))
    else
      ShowMessage('David n''est pas dans le dictionnaire');
    ...
    // Modifier une valeur (ou la créer si la clé n'existe pas avec AddOrSetValue)
    Ages['Bob'] := 31;
    Ages.AddOrSetValue('Eve', 22);  // Ajoute Eve, ou remplace son âge
...
> ⚠️ **Ordre des éléments** : Contrairement à un tableau, un `TDictionary` **ne garantit pas** l'ordre d'insertion. Si vous avez besoin d'un ordre d'itération stable, utilisez `TObjectDictionary` (similaire) avec un `TList<TKey>` parallèle, ou cherchez d'autres structures (`TSortedDictionary` dans certaines bibliothèques tierces).
...
begin
  ...
  try
    ...
    ComparerParAge := TComparer<TPersonne>.Construct(
      ...
      begin
        // CompareValue retourne -1, 0 ou 1 sans risque de débordement
        Result := CompareValue(A.Age, B.Age);
      end
    );
...
2. **Pas de valeurs par défaut pour T** : on ne peut pas écrire `FValeur: T = DefaultValue` à la déclaration. Pour obtenir la valeur par défaut d'un type générique (`0` pour les entiers, `''` pour les chaînes, `nil` pour les références), utilisez la fonction `Default(T)` :

   ```pascal
   procedure TMaClasse<T>.Reinitialiser;
   begin
     FValeur := Default(T);  // 0, '', nil, etc. selon T
   end;
   ```

3. **Contraintes limitées** : les contraintes sont moins puissantes que dans d'autres langages (par exemple, pas de contrainte d'opérateur arithmétique).
