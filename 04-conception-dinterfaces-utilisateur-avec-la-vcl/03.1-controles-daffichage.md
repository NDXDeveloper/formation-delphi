...
```pascal
Label1.Transparent := True;  // Transparent
Label1.Transparent := False; // Opaque (par défaut)
```

**Note :** Avec Transparent à True, vous verrez la couleur du formulaire derrière le texte.

> 💡 **Pourquoi un TLabel semble-t-il déjà transparent par défaut ?** Bien que `Transparent` soit à `False` par défaut, la propriété `ParentColor` est, elle, à `True`. Le label adopte donc la couleur de son parent, ce qui donne **l'illusion** de transparence sur un fond uni. Si vous placez un label au-dessus d'une image ou d'un dégradé, vous verrez que son fond est en réalité opaque — c'est à ce moment-là qu'il faut passer `Transparent` à `True`.
...
Avec ShowAccelChar à True, le raccourci Alt+N déplacera le focus vers le contrôle désigné par la propriété **FocusControl** du label (voir la propriété suivante).
...
var
  A, B, Total: Double;
begin
  // Utiliser TryStrToFloat pour éviter une exception si l'utilisateur
  // saisit du texte non numérique.
  if not TryStrToFloat(Edit1.Text, A) or not TryStrToFloat(Edit2.Text, B) then
  begin
    lblResultat.Caption := 'Saisie invalide';
    lblResultat.Font.Color := clRed;
    Exit;
  end;
...
**Recommandations :**
- Texte normal : 9-11 points (minimum 9 sur les écrans haute résolution)
- Titres : 12-16 points
...
```pascal
// Charger depuis un chemin absolu
Image1.Picture.LoadFromFile('C:\Images\logo.png');

// Charger depuis un chemin relatif au dossier de l'exécutable (recommandé)
...
> 💡 **Différence chemin relatif vs ressource intégrée** : les exemples ci-dessus chargent une image depuis un **fichier externe**. Pour intégrer une image **directement dans l'exécutable** (et ne pas dépendre d'un fichier accompagnant l'`.exe`), il faut passer par un fichier ressource (`.res`) ou par `Image1.Picture.LoadFromResourceName(HInstance, 'MON_LOGO')`. Ce point sera détaillé plus loin dans la formation.
...
> 💡 **Astuce — activer le bon format** : la prise en charge des formats JPG, PNG et GIF nécessite d'ajouter les unités correspondantes dans la clause `uses` de votre unité :
>
> ```pascal
> uses
>   Vcl.Imaging.jpeg,   // pour les .jpg / .jpeg
>   Vcl.Imaging.pngimage, // pour les .png
>   Vcl.Imaging.GIFImg;   // pour les .gif (statiques et animés)
> ```
>
> Si vous oubliez ces unités, le chargement d'un PNG ou JPG lèvera une exception `EInvalidGraphic`.
...
```pascal
// Nécessite : uses Winapi.ShellAPI;
...
2. **Taille lisible** : Minimum **9 points** (idéalement 10-11) pour le texte normal ; les écrans modernes haute résolution rendent 8 pt trop petit
3. **Contraste** : Assurez un bon contraste pour la lisibilité (ratio ≥ 4.5:1 selon WCAG AA)
