...
begin  
  // Autoriser chiffres, virgule, point (sera converti) et backspace
  if not CharInSet(Key, ['0'..'9', ',', '.', #8]) then
    Key := #0;  // Ignorer la touche

  // Normaliser : remplacer le point par une virgule
  // (sur un clavier numérique, le séparateur est souvent un point)
  if Key = '.' then
    Key := ',';
end;
...
```pascal
// Nécessite : uses Winapi.Messages;  // pour la constante EM_SCROLLCARET
...
> ⚠️ **Attention à la fuite mémoire** : `AddObject` n'établit aucun lien de propriété ; la `TStringList` interne **ne libère pas** les objets quand vous appelez `Items.Clear`, `Items.Delete` ou même `Free` sur la ComboBox. Vous devez libérer manuellement chaque objet avant d'effacer la liste, sinon vous créez une fuite mémoire à chaque rechargement.
