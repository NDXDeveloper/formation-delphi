...
#### 2. Texte clair et orienté action

Le texte du bouton doit indiquer clairement l'action effectuée :

**Bon :** "Enregistrer", "Supprimer", "Rechercher" (verbes à l'infinitif décrivant l'action)
**Mauvais :** "OK", "Faire", "Cliquer ici" (vagues ou redondants)
...
Indique combien d'images sont **juxtaposées horizontalement** dans le bitmap `Glyph`. Chaque image correspond à un état du bouton (1 à 4) :

```pascal
BitBtn1.NumGlyphs := 1;  // Une seule image utilisée pour tous les états (par défaut)
BitBtn1.NumGlyphs := 2;  // Normal + Désactivé
BitBtn1.NumGlyphs := 3;  // Normal + Désactivé + Pressé
BitBtn1.NumGlyphs := 4;  // Normal + Désactivé + Pressé + Stay-Down (sélectionné)
```

> 📌 **Précision** : la quatrième image correspond à l'état « **Stay-Down** » (bouton qui reste enfoncé, p. ex. dans une barre d'outils en mode bascule), **pas au survol** de la souris. Les images doivent être de même largeur et alignées horizontalement dans le bitmap source. Le pixel en bas à gauche du bitmap définit la couleur de transparence.
...
> 💡 **Le « groupe » est défini par le contrôle parent** : tous les `TRadioButton` qui partagent le même conteneur direct (le formulaire, un `TPanel` ou un `TGroupBox`) appartiennent automatiquement au même groupe. Pour obtenir **plusieurs groupes indépendants** sur un même formulaire, placez chaque ensemble dans un `TGroupBox` ou un `TPanel` distinct — sinon tous vos RadioButtons seront mutuellement exclusifs.
...
var
  FormatChoisi: string;   // Évitez `Format` : c'est le nom d'une fonction RTL
begin
