...
// Supprimer un élément par valeur (vérifier qu'il existe avant !)
Idx := ListBox1.Items.IndexOf('Rouge');
if Idx >= 0 then
...
```pascal
ListBox1.IntegralHeight := True;   // Pas d'éléments partiellement visibles
ListBox1.IntegralHeight := False;  // Hauteur exacte définie (par défaut)
```

> 💡 **Effet visuel** : avec `IntegralHeight := True`, la liste raccourcit légèrement sa hauteur réelle pour qu'aucun item ne soit coupé en bas. C'est plus joli mais cela peut surprendre la première fois — la hauteur que vous fixez n'est pas toujours strictement respectée.
...
begin
  ...
  try
    ...
    begin
      ...
      for j := 0 to StringGrid1.ColCount - 1 do
      begin
        Cellule := StringGrid1.Cells[j, i];

        // Échapper les cellules contenant des virgules, guillemets ou retours à la ligne
        if (Pos(',', Cellule) > 0) or (Pos('"', Cellule) > 0) or
           (Pos(#13, Cellule) > 0) or (Pos(#10, Cellule) > 0) then
          Cellule := '"' + StringReplace(Cellule, '"', '""', [rfReplaceAll]) + '"';
    ...
    // Enregistrer en UTF-8 (avec BOM) pour la compatibilité Excel et l'international
    Sortie.SaveToFile(NomFichier, TEncoding.UTF8);
  finally
    Sortie.Free;
  end;
end;
...
> 💡 **Pourquoi ces précautions ?** Le format CSV impose d'**échapper** les valeurs contenant le séparateur (`,`), des guillemets ou des retours à la ligne — sans cela, un nom comme `Dupont, Jean` casserait la structure du fichier. L'écriture en UTF-8 avec BOM garantit que les caractères accentués s'affichent correctement quand Excel ou un autre tableur ouvre le CSV.
...
begin
  ...
  try
    repeat
      ...
      // SearchRec.TimeStamp est un TDateTime (recommandé en Delphi moderne).
      // L'ancien SearchRec.Time est déprécié.
      Item.SubItems.Add(DateTimeToStr(SearchRec.TimeStamp));
    until FindNext(SearchRec) <> 0;
  finally
    FindClose(SearchRec);  // Toujours libérer le handle, y compris en cas d'exception
  end;
end;
```
...
Le tri d'un `TListView` se fait via l'événement `OnCompare`, déclenché par `AlphaSort`. On garde l'index de la colonne courante et le sens du tri dans des champs privés du formulaire :
...
begin
  // Inverser le sens si on reclique la même colonne, sinon partir en croissant
  ...
  ListView1.AlphaSort;  // Déclenche OnCompare pour chaque paire
end;
...
var
  S1, S2: string;
begin
  // Colonne 0 = Caption ; les autres = SubItems[FSortColumn - 1]
