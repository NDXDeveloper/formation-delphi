...
begin  
  ...
  {$IFDEF LINUX}
  // Répertoire XDG standard : ~/.local/share/NotesApp
  // (TPath.Combine accepte 2, 3 ou 4 paramètres — pas besoin d'appels imbriqués)
  FCheminDonnees := TPath.Combine(TPath.GetHomePath, '.local', 'share', 'NotesApp');
  {$ELSE}
  FCheminDonnees := TPath.Combine(TPath.GetDocumentsPath, 'NotesApp');
  {$ENDIF}
  ...
  try
    ...
    try
      ...
      begin
        ...
        // TJSONObject.GetValue<T> ne supporte pas TDateTime directement ;
        // on lit comme Double (TDateTime = Double sous le capot)
        Note.DateCreation := JSONNote.GetValue<Double>('dateCreation');
        Note.DateModification := JSONNote.GetValue<Double>('dateModification');
  ...
  try
    try
      ...
      begin
        ...
        JSONNote.AddPair('dateCreation',
          TJSONNumber.Create(Note.DateCreation));
        JSONNote.AddPair('dateModification',
          TJSONNumber.Create(Note.DateModification));
    ...
    JSON.Free;  // Libération garantie même en cas d'exception
  end;
end;
...
uses
  ...
  {$IFDEF LINUX}
  Posix.SysUtsname, Posix.Stdlib, Posix.SysStatvfs,
  {$ENDIF}
  System.IOUtils;
...
begin
  ...
  try
    // ⚠️ TDirectory.GetDiskSpace / GetDiskFreeSpace n'existent pas en
    // standard. Sur Linux, on utilise l'API POSIX statvfs.
    if statvfs(PAnsiChar(AnsiString(TPath.GetHomePath)), Buf) = 0 then
    begin
      TotalBytes := Int64(Buf.f_blocks) * Int64(Buf.f_frsize);
      FreeBytes  := Int64(Buf.f_bavail) * Int64(Buf.f_frsize);
...
type
  // ⚠️ Important : on évite `TApplication` (déjà défini dans FMX.Forms
  // comme variable globale Application), pour ne pas masquer le symbole.
  TLauncherApp = record
    Nom: string;
    Commande: string;
    Icone: string;
...
begin  
  ...
  Result := TPath.Combine(TPath.GetHomePath, '.config', 'QuickLauncher', 'apps.json');
  {$ELSE}
  Result := TPath.Combine(TPath.GetDocumentsPath, 'QuickLauncher', 'apps.json');
  {$ENDIF}
end;
...
procedure TFormLauncher.CreerBoutonApp(const App: TLauncherApp);  
var  
  Layout: TLayout;
  Rect: TRectangle;
...
begin
  ...
  // Événement clic — on stocke la commande dans le TagString du Rect
  // pour pouvoir la retrouver depuis Sender dans le handler.
  // (Note : OnClick est un TNotifyEvent (`of object`) — il n'accepte
  //  PAS une procédure anonyme inline, d'où la méthode d'instance.)
  Rect.TagString := App.Commande;
  Rect.OnClick := RectAppClick;
end;

procedure TFormLauncher.RectAppClick(Sender: TObject);  
begin  
  if Sender is TRectangle then
    LancerApplication(TRectangle(Sender).TagString);
end;
...
var  
  ...
  App: TLauncherApp;
begin
  // ℹ️ La signature multi-champs `InputQuery` (FMX.Dialogs) prend les
  // valeurs en paramètre VAR — on doit donc passer une vraie variable
  // d'array, pas un littéral comme `[Nom, Commande, Catégorie]`.
  SetLength(Valeurs, 3);
  Valeurs[0] := '';        // valeurs initiales
  Valeurs[1] := '';
  Valeurs[2] := 'Autres';  // catégorie par défaut

  if InputQuery('Nouvelle application',
                ['Nom', 'Commande', 'Catégorie'],
                Valeurs) then
  begin
    if (Valeurs[0] <> '') and (Valeurs[1] <> '') then
    begin
      App.Nom := Valeurs[0];
      App.Commande := Valeurs[1];
      App.Categorie := Valeurs[2];
      App.Icone := '';
...
CheminConfig := TPath.Combine(TPath.GetHomePath, '.config', 'MonApp');

// Données : ~/.local/share/MonApp
CheminDonnees := TPath.Combine(TPath.GetHomePath, '.local', 'share', 'MonApp');

// Cache : ~/.cache/MonApp
CheminCache := TPath.Combine(TPath.GetHomePath, '.cache', 'MonApp');
{$ENDIF}
```
