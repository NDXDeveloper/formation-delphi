...
**SQL** (prononcé « ess-cu-elle » en français, ou « sii-couel » à l'anglaise) est un langage standardisé pour communiquer avec les bases de données relationnelles. C'est comme un langage universel que comprennent MySQL, PostgreSQL, SQL Server, Oracle et la plupart des bases de données.
...
begin  
  ...
  // Paramètre date — préférer EncodeDate à StrToDate, car StrToDate dépend
  // des paramètres régionaux (FormatSettings) : '01/01/2024' peut être le
  // 1er janvier en France et le 1er avril en anglais US (mm/dd/yyyy).
  FDQueryClients.ParamByName('DateDebut').AsDate := EncodeDate(2024, 1, 1);
...
> 💡 **Limitation des `IN` avec paramètres** : SQL standard ne permet pas de passer un nombre **variable** de valeurs dans un seul paramètre — chaque emplacement `:NomN` doit être un paramètre nommé distinct. Pour une liste dynamique (par exemple un `TStringList` saisi par l'utilisateur), trois approches courantes :  
>  
> 1. **Construire le SQL avec le bon nombre de placeholders** :  
>  
>    ```pascal  
>    var SQL: string; i: Integer;  
>    SQL := 'SELECT * FROM clients WHERE nom IN (';  
>    for i := 0 to Noms.Count - 1 do  
>    begin  
>      if i > 0 then SQL := SQL + ',';  
>      SQL := SQL + ':Nom' + IntToStr(i);  
>    end;  
>    SQL := SQL + ')';  
>    FDQueryClients.SQL.Text := SQL;  
>    for i := 0 to Noms.Count - 1 do  
>      FDQueryClients.ParamByName('Nom' + IntToStr(i)).AsString := Noms[i];  
>    ```  
>    Sûr contre l'injection car les **valeurs** restent dans des paramètres.  
>  
> 2. **Macros FireDAC** (`!`) : construction dynamique côté FireDAC, plus simple à écrire.  
> 3. **Table temporaire** : insérer les valeurs dans une table temp et faire un `JOIN` — recommandé pour de grandes listes (> 100 éléments).
...
```pascal
// ⚠️ Important : SQL.Text contient la requête AVEC les :paramètres,
// pas les valeurs substituées (et c'est tant mieux — c'est ce qui rend
// les paramètres sûrs face aux injections). Pour visualiser les valeurs
// effectivement envoyées au serveur, listez chaque paramètre séparément.
ShowMessage(FDQueryClients.SQL.Text);  
ShowMessage('Paramètre Nom : ' + FDQueryClients.ParamByName('Nom').AsString);  
```

> 🔍 **Voir la requête exacte envoyée au serveur** : la requête envoyée par FireDAC reste **paramétrée** (les valeurs voyagent à part) — on ne peut donc pas reconstruire une seule chaîne SQL "finale". Pour observer en temps réel ce qui transite, activez le moniteur FireDAC :  
>  
> - Déposez un `TFDMoniRemoteClientLink` sur votre formulaire (ou ajoutez `FDConnection1.Params.Add('MonitorBy=Remote')`)  
> - Lancez l'outil **FDMonitor.exe** fourni avec Delphi (généralement dans `bin\FDMonitor.exe` de l'installation Delphi)  
>  
> FDMonitor affiche chaque requête avec ses paramètres résolus, les temps d'exécution et les erreurs — précieux pour diagnostiquer les performances et les bugs subtils.
