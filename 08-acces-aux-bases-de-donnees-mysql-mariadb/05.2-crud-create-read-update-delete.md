...
begin  
  ...
  try
    ...
    // Note : ces accès supposent que Columns.Count >= 5 (auto-générées par
    // FireDAC à partir des champs du SELECT). Si vous avez défini les colonnes
    // au design-time via le Columns Editor, leur ordre est figé et indépendant
    // du SELECT — adaptez les indices en conséquence.
    if DBGridClients.Columns.Count >= 5 then
    begin
...
uses
  // ...
  uDmDatabase;  // Ajouter l'unité du DataModule dans uses

implementation

procedure TFormGestionClients.FormCreate(Sender: TObject);  
begin  
  // Accéder au DataModule (nom de l'instance globale créée par l'IDE)
  dmDatabase.FDConnection1.Connected := True;
  ChargerClients;
end;

procedure TFormGestionClients.ChargerClients;  
begin  
  with dmDatabase.FDQueryClients do
  begin
    Close;
...
> 💡 **Convention de nommage** : par convention, l'unité s'appelle `uDmDatabase.pas` (préfixe `u` pour unit, `Dm` pour DataModule), la classe `TdmDatabase`, et l'instance globale `dmDatabase`. Adaptez selon les conventions de votre équipe — l'essentiel est la cohérence à travers tout le projet.
...
begin  
  ...
  try
    ...
    on E: Exception do
    begin
      // Erreur : tout annuler ET propager pour que l'appelant soit informé
      FDConnection1.Rollback;
      ShowMessage('Transfert annulé : ' + E.Message);
      raise;  // ⚠️ Important : sans raise, l'appelant croit que tout s'est bien passé
    end;
  end;
end;
```

> ⚠️ **Le `raise` final est crucial** : sans lui, la procédure « avale » silencieusement les exceptions après le rollback. L'appelant pense que le transfert a réussi, alors qu'il a été annulé. Cette différence peut produire des bugs très difficiles à diagnostiquer en production. **Toujours** propager l'exception après un rollback (sauf si vous avez une raison explicite de ne pas le faire et que vous retournez un code d'erreur).
...
Ajouter une fonction d'export CSV. On utilise `TStringList` (qui gère UTF-8) plutôt que `TextFile` (qui force l'encodage système), et on échappe correctement les champs selon RFC 4180 :

```pascal
uses
  System.Classes, System.SysUtils, System.StrUtils;

// Échappement RFC 4180 : si le champ contient un séparateur, un guillemet
// ou un saut de ligne, on l'entoure de guillemets et on double les guillemets internes.
...
begin
  ...
  try
    ...
    begin
      ...
      try
        // En-têtes
        Lignes.Add('ID;Nom;Prénom;Email;Téléphone');

        // Données — important : utiliser AsString sur le Field pour la conversion
        // explicite, et échapper chaque champ contre les caractères spéciaux.
        FDQueryClients.First;
        while not FDQueryClients.Eof do
        begin
          Lignes.Add(
            EchapperCSV(FDQueryClients.FieldByName('id').AsString) + ';' +
            EchapperCSV(FDQueryClients.FieldByName('nom').AsString) + ';' +
            EchapperCSV(FDQueryClients.FieldByName('prenom').AsString) + ';' +
            EchapperCSV(FDQueryClients.FieldByName('email').AsString) + ';' +
            EchapperCSV(FDQueryClients.FieldByName('telephone').AsString)
          );
          FDQueryClients.Next;
        end;

        // Sauvegarde en UTF-8 (avec BOM pour Excel)
        Lignes.SaveToFile(SaveDialog.FileName, TEncoding.UTF8);
...
> ⚠️ **Pièges à éviter dans l'export CSV** :  
>  
> - **Ne pas utiliser `Query['champ']`** : la notation crochets retourne un `Variant`. La concaténation `Variant + ';' + Variant` lève une `EVariantTypeCastError` dès que le champ est un Integer, une Date ou un NULL. Préférez `FieldByName('x').AsString` qui fait la conversion correctement.  
> - **Ne pas utiliser `TextFile` (Write/WriteLn)** : sur Windows, `TextFile` écrit en encodage ANSI système, ce qui casse les caractères accentués (é, à, ç…) à l'ouverture sous un autre OS ou un autre éditeur. `TStringList.SaveToFile(..., TEncoding.UTF8)` règle le problème et ajoute le BOM UTF-8 que beaucoup d'outils (Excel, LibreOffice) attendent.  
> - **Toujours échapper les champs** : un nom comme `"Société; SARL"` casse un CSV non échappé. Voir le chapitre 7.8 pour les détails CSV/Excel.
