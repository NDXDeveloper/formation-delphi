...
> ⚠️ **Prérequis MySQL — le moteur de stockage** : les transactions ne fonctionnent qu'avec un moteur de stockage **transactionnel**. Sur MySQL/MariaDB, vos tables doivent être en **InnoDB** (le moteur par défaut depuis MySQL 5.5 et MariaDB 10.2). Si une table est en **MyISAM** (vieux moteur), `StartTransaction`/`Commit`/`Rollback` s'exécutent **sans erreur mais sans effet** — vos modifications sont écrites immédiatement et le `Rollback` est silencieusement ignoré ! Pour vérifier le moteur :  
>  
> ```sql  
> -- Voir le moteur de toutes les tables  
...
> ```  
>  
> Bonne nouvelle : si vous avez créé vos tables depuis HeidiSQL ou via les exemples de ce chapitre, elles sont déjà en InnoDB par défaut.
...
> ⚠️ **Support des contraintes CHECK selon la version** :  
>  
> - **MariaDB 10.2+** : CHECK appliquées correctement.  
> - **MySQL 8.0.16+** : CHECK appliquées correctement.  
> - **MySQL < 8.0.16** : la syntaxe est **acceptée mais ignorée silencieusement** — vos contraintes ne s'appliquent pas ! Vérifiez votre version avec `SELECT VERSION();`.  
>  
> Limitation supplémentaire : les CHECK ne peuvent pas utiliser de **fonctions non déterministes** (`NOW()`, `CURDATE()`…) — ces fonctions retournent une valeur différente à chaque appel et ne sont pas autorisées dans une contrainte CHECK qui doit être déterministe. Pour une règle « date dans le passé », utilisez plutôt un **TRIGGER** :  
>  
> ```sql  
> -- MySQL/MariaDB : trigger pour valider la date d'embauche  
> DELIMITER //  
> CREATE TRIGGER check_date_embauche BEFORE INSERT ON employes  
> FOR EACH ROW  
> BEGIN  
>   IF NEW.date_embauche > CURDATE() THEN  
>     SIGNAL SQLSTATE '45000'  
>       SET MESSAGE_TEXT = 'date_embauche ne peut pas être dans le futur';  
>   END IF;  
> END //  
> DELIMITER ;  
> ```
...
// tentative 1 → 100ms..200ms, tentative 2 → 200ms..400ms, tentative 3 → 400ms..800ms…
// Le jitter évite que toutes les transactions concurrentes retentent en même temps.
procedure ExecuterAvecRetry(const Operation: TProc; MaxTentatives: Integer = 3);  
var  
  Tentative: Integer;
  DelaiBase, DelaiAttente: Integer;
begin
  for Tentative := 1 to MaxTentatives do
  begin
    try
      Operation();
      Exit;  // Succès : on quitte
    except
      on E: Exception do
      begin
        ...
        if (Pos('deadlock', LowerCase(E.Message)) = 0) or
           (Tentative = MaxTentatives) then
          raise;

        // Sinon : backoff exponentiel avec jitter
        DelaiBase := 100 * (1 shl (Tentative - 1));  // 100, 200, 400, 800ms…
        DelaiAttente := DelaiBase + Random(DelaiBase);
        Sleep(DelaiAttente);
      end;
    end;
  end;
...
// ⚠️ N'oubliez pas d'appeler Randomize une seule fois au démarrage de l'app
// (par exemple dans le bloc initialization de l'unité principale ou dans
// le .dpr) — sans cela, Random retourne toujours la même séquence et
// le jitter perd son intérêt.
initialization
  Randomize;
...
> 💡 **Pour aller plus loin sur le logging** : cet exemple basique utilise `TextFile` (encodage ANSI système sous Windows) et n'est **pas thread-safe**. Pour une application en production multi-thread ou avec des messages accentués, voir le chapitre 8.11 qui présente une version `TAuditLogger` complète avec `TCriticalSection` (sérialisation des écritures) et `TFileStream` + `TEncoding.UTF8` (encodage portable).
