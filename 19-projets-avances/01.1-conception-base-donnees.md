...
- id_commande (INT, clé étrangère vers COMMANDES, UNIQUE pour la relation 1:1)
- id_client (INT, clé étrangère vers CLIENTS)
- date_facture (DATE)
- date_echeance (DATE)
- montant_ht (DECIMAL(10,2))
...
> Le `UNIQUE` sur `id_commande` est essentiel pour garantir techniquement la relation 1:1 : sans lui, rien n'empêche la création de deux factures pour la même commande.
...
> ⚠️ Les contraintes `CHECK` ne sont **réellement appliquées** qu'à partir de **MySQL 8.0.16** et **MariaDB 10.2**. Sur les versions antérieures, MySQL accepte la syntaxe mais ignore la contrainte silencieusement. Vérifiez votre version avec `SELECT VERSION();` avant de vous fier à ces vérifications.
