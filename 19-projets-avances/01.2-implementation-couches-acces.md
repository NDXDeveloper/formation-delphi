...
L'implémentation des couches d'accès aux données est l'étape qui transforme notre conception de base de données en code Delphi fonctionnel. Cette couche fait le lien entre votre interface utilisateur et la base de données MySQL/MariaDB. Une bonne architecture des couches d'accès rend votre application plus maintenable, testable et évolutive.

## Qu'est-ce qu'une couche d'accès aux données ?

Une couche d'accès aux données (DAL - Data Access Layer) est un ensemble de classes et de modules qui :
- Encapsulent toutes les interactions avec la base de données
- Isolent le code métier de la technologie d'accès aux données
- Fournissent une API claire et simple pour manipuler les données
- Gèrent les connexions, transactions et erreurs

### Avantages de cette approche

**Séparation des responsabilités** : Le code de l'interface utilisateur ne contient pas de SQL, le rendant plus lisible.

**Réutilisabilité** : Les mêmes méthodes d'accès peuvent être utilisées dans différentes parties de l'application.

**Testabilité** : Vous pouvez tester la logique d'accès aux données indépendamment de l'interface.

**Maintenabilité** : Les modifications de la base de données sont centralisées dans la couche d'accès.

**Changement de SGBD facilité** : Si vous devez changer de base de données, seule la couche d'accès doit être modifiée.

## Architecture en couches

Voici l'architecture recommandée pour notre application :

```
┌─────────────────────────────────┐
│   Couche Présentation (UI)      │  ← Formulaires VCL/FMX
│   - Forms                       │
│   - Validation interface        │
└──────────────┬──────────────────┘
               │
┌──────────────▼──────────────────┐
│   Couche Métier (Business)      │  ← Logique métier
│   - Règles de gestion           │
│   - Calculs                     │
│   - Validations métier          │
└──────────────┬──────────────────┘
               │
┌──────────────▼──────────────────┐
│   Couche Accès Données (DAL)    │  ← Notre focus ici
│   - Modules de données          │
│   - Classes DAO                 │
│   - Gestion connexions          │
└──────────────┬──────────────────┘
...
## FireDAC : Le framework d'accès aux données de Delphi

FireDAC est le framework moderne d'accès aux données de Delphi. Il offre :
...
- Cache local et mode déconnecté
- Paramètres typés et protection contre l'injection SQL

### Composants FireDAC essentiels

**FDConnection** : Gère la connexion à la base de données

**FDQuery** : Exécute des requêtes SELECT et récupère des données

**FDCommand** : Exécute des commandes SQL (INSERT, UPDATE, DELETE) sans retour de données

**FDStoredProc** : Appelle des procédures stockées

**FDTransaction** : Gère les transactions

**FDPhysMySQLDriverLink** : Active le support MySQL/MariaDB

## Étape 1 : Configuration de la connexion

### 1.1 Créer un module de données de connexion

Le module de données centralise la connexion à la base de données.
...
begin  
  ...
  try
    ...
    on E: Exception do
      raise Exception.Create('Erreur lors du démarrage de la transaction : ' + E.Message);
  end;
end;
...
### 1.2 Amélioration : Paramètres de connexion depuis un fichier

Pour plus de flexibilité, stockez les paramètres dans un fichier INI :
...
## Étape 2 : Créer des classes entités

Les classes entités représentent les tables de la base de données sous forme d'objets Pascal.
...
## Étape 3 : Le pattern DAO (Data Access Object)

Le pattern DAO encapsule l'accès aux données pour chaque entité.
...
type
  // Note importante : `TInterfacedObject` permet d'implémenter `IBaseDAO<TClient>`.
  // Comme nous utilisons toujours la classe comme un objet (jamais via une variable
  // de type interface), nous appelons `Free` explicitement. Si vous décidez de
  // stocker un DAO dans une variable d'interface (ex. `IDAO: IBaseDAO<TClient>`),
  // ne mélangez JAMAIS objet et interface pour le même DAO — le comptage de
  // références d'`IInterface` déclencherait alors un double `Destroy`.
...
begin
  ...
  try
    ...
    // Ou suppression physique (décommenter si nécessaire)
...
begin
  if not dmConnection.StartTransaction then
    raise Exception.Create('Impossible de démarrer la transaction');

  try
    // Try/finally imbriqués pour libérer chaque DAO même si la création
    // du suivant lève une exception.
...
## Étape 5 : Le pattern Repository

Le pattern Repository offre une abstraction supplémentaire au-dessus des DAO.
...
type
  ...
  public
    ...
    function ValidateClient(AClient: TClient; AErrors: TStringList): Boolean;
...
begin
  ...
  try
    if not ValidateClient(AClient, Errors) then
      raise Exception.Create('Données client invalides :' + sLineBreak + Errors.Text);
  finally
    Errors.Free;
  end;
...
begin
  ...
  if (Trim(AClient.Email) <> '') and (Pos('@', AClient.Email) = 0) then
    AddError('Format d''email invalide');

  // Validation du type client
  if AClient.IsEntreprise and (Trim(AClient.RaisonSociale) = '') then
    AddError('La raison sociale est obligatoire pour une entreprise');
...
## Étape 6 : Utilisation dans l'interface utilisateur
...
begin  
  ...
  FClients := nil;  // sera créé par LoadClients
  ...
  gridClients.Cells[3, 0] := 'Prénom';
  gridClients.Cells[4, 0] := 'Email';
  gridClients.Cells[5, 0] := 'Téléphone';
...
begin
  ...
  try
    FormEdit.ClientId := 0;  // 0 = nouveau client (convention partagée avec 19.1.3)
...
begin
  ...
  FormEdit := TFormClientEdit.Create(Self);
  try
    FormEdit.ClientId := ClientId;  // > 0 = édition du client existant
    if FormEdit.ShowModal = mrOk then
      LoadClients;
  finally
    FormEdit.Free;
...
begin
  ...
  try
    ...
    Client.Free;  // Libération garantie
  end;
end;
```
...
**Capturer et gérer les exceptions appropriées** :
```pascal
try
...
**DAO** : Accès aux données pur (CRUD)  
**Repository** : Logique métier et orchestration  
**UI** : Présentation et interaction utilisateur  

### 5. Tests unitaires

Créez des tests pour vos DAO et Repository :
```pascal
...
begin
  ...
  try
    ...
    try
      ...
      Assert(Client.IdClient > 0, 'Échec de l''insertion');

      // Test récupération
      var ClientRecup := DAO.GetById(Client.IdClient);
      try
        Assert(ClientRecup.CodeClient = 'TEST001', 'Données incorrectes');
      finally
        ClientRecup.Free;
      end;

      // Test suppression
      Assert(DAO.Delete(Client.IdClient), 'Échec de la suppression');
    finally
      Client.Free;
    end;
  finally
...
MonProjet/
├── Database/
│   ├── uDMConnection.pas         // Module de connexion
│   └── uDMConnection.dfm
├── Entities/
│   ├── uClient.pas               // Classes entités
│   ├── uProduit.pas
│   ├── uCommande.pas
│   └── ...
├── DAO/
│   ├── uBaseDAO.pas              // Interface de base
│   ├── uClientDAO.pas            // DAO spécifiques
│   ├── uProduitDAO.pas
│   └── ...
├── Repository/
│   ├── uClientRepository.pas     // Repositories
│   ├── uProduitRepository.pas
│   └── ...
├── Business/
│   └── uTransactionHelper.pas    // Helpers et utilitaires
└── UI/
    ├── Forms/
    │   ├── uFormClients.pas      // Formulaires
    │   └── uFormClients.dfm
    └── ...
```
...
L'implémentation d'une couche d'accès aux données structurée avec FireDAC vous permet de créer des applications robustes et maintenables. Les patterns DAO et Repository offrent une séparation claire des responsabilités et facilitent l'évolution de votre application.

Les points clés à retenir :
- Centralisez la connexion dans un module dédié
- Utilisez des classes entités pour représenter vos tables
- Implémentez des DAO pour chaque entité
- Ajoutez une couche Repository pour la logique métier
- Utilisez des transactions pour les opérations multiples
- Toujours utiliser des paramètres dans les requêtes SQL
- Gérez correctement les ressources et exceptions
- Documentez votre code

Dans le prochain chapitre, nous verrons comment créer une interface utilisateur évoluée qui exploite cette couche d'accès aux données.
