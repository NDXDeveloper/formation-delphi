...
Une interface utilisateur bien conçue est cruciale pour le succès d'une application. Elle doit être intuitive, réactive, esthétique et offrir une excellente expérience utilisateur. Dans ce chapitre, nous allons créer des interfaces professionnelles en utilisant la VCL (Visual Component Library) de Delphi et en exploitant les couches d'accès aux données que nous avons développées.

## Principes de conception d'interface

### 1. Les fondamentaux de l'UX (User Experience)

**Clarté** : L'utilisateur doit comprendre immédiatement ce qu'il peut faire et comment le faire.

**Cohérence** : Les éléments similaires doivent se comporter de manière similaire dans toute l'application.

**Feedback** : Chaque action de l'utilisateur doit avoir une réponse visible (message, changement visuel, son).

**Efficacité** : Minimiser le nombre de clics et d'étapes nécessaires pour accomplir une tâche.

**Prévention des erreurs** : Valider les données en temps réel et guider l'utilisateur.

**Accessibilité** : L'interface doit être utilisable par tous, y compris les personnes avec des besoins spécifiques.

### 2. Organisation visuelle

**Hiérarchie visuelle** : Les éléments importants doivent être plus visibles (taille, couleur, position).

**Espacement** : Utilisez des marges et des espacements généreux pour aérer l'interface.

**Alignement** : Alignez les contrôles pour créer une structure visuelle claire.

**Groupement** : Regroupez les éléments liés dans des panels ou des GroupBox.

**Couleurs** : Utilisez une palette de couleurs cohérente et professionnelle.
...
├── Menu principal
├── Barre d'outils
├── Zone de contenu (pages ou formulaires enfants)
│   ├── FormClients
│   ├── FormProduits
│   ├── FormCommandes
│   ├── FormFactures
...
Le formulaire principal est le point d'entrée de l'application.
...
begin  
  ...
  try
    ...
    UpdateStatusBar('Connecté à la base de données');
...
### 1. Formulaire de gestion des clients (avancé)

Créons un formulaire moderne avec toutes les fonctionnalités attendues.
...
type
  TFormClients = class(TForm)
    ...
    procedure btnPagePrecedenteClick(Sender: TObject);
    procedure btnPageSuivanteClick(Sender: TObject);
...
begin  
  // Configuration de la requête FireDAC
  ...
  // Note : TDBGrid standard n'a pas de propriété AlternatingRowBackground.
  // On câble notre handler OnDrawColumnCell pour dessiner une ligne sur deux
  // en gris clair (le composant TDBGridEh ou TcxGrid fournirait cette option nativement).
  DBGrid.OnDrawColumnCell := DBGridDrawColumnCell;
end;

procedure TFormClients.DBGridDrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  // Alternance de couleurs : 1 ligne sur 2 en gris très clair
  if Odd(FDQuery.RecNo) then
    DBGrid.Canvas.Brush.Color := clWhite
  else
    DBGrid.Canvas.Brush.Color := $00F5F5F5;
...
begin
  ...
  if ClientId = 0 then
  begin
    MessageDlg('Veuillez sélectionner un client à modifier',
...
begin
  ...
  if ClientId = 0 then
  begin
    MessageDlg('Veuillez sélectionner un client à supprimer',
      mtInformation, [mbOK], 0);
    Exit;
  end;
  ...
  try
    if MessageDlg(
      Format('Êtes-vous sûr de vouloir supprimer le client "%s" ?',
    ...
    begin
      try
        ...
        begin
          MessageDlg('Client supprimé avec succès', mtInformation, [mbOK], 0);
          ChargerClients;
        end
        else
...
procedure TFormClients.btnPagePrecedenteClick(Sender: TObject);  
begin  
  if FPageActuelle > 1 then
  begin
    Dec(FPageActuelle);
    ChargerClients;
  end;
...
procedure TFormClients.btnPageSuivanteClick(Sender: TObject);  
var  
  TotalPages: Integer;
begin
  TotalPages := (FTotalClients + FNombreParPage - 1) div FNombreParPage;
  if FPageActuelle < TotalPages then
  begin
    Inc(FPageActuelle);
    ChargerClients;
  end;
...
begin
  ...
  begin
    MessageDlg('Aucune donnée à exporter', mtInformation, [mbOK], 0);
    Exit;
  end;
  ...
  try
    ...
    try
      ...
      MessageDlg('Excel n''est pas installé sur cet ordinateur',
        mtError, [mbOK], 0);
      Exit;
    end;
    ...
    MessageDlg('Export vers Excel réussi', mtInformation, [mbOK], 0);
...
### 1. Formulaire d'édition de client
...
begin  
  ...
  // Configuration du MaskEdit pour le téléphone
...
begin
  try
    // Charger d'abord, puis remplacer seulement si succès — évite un FClient dangling en cas d'exception
    Loaded := FRepository.GetClient(FClientId);
    FClient.Free;
    FClient := Loaded;
...
begin  
  // FClient est supposé déjà rempli par le caller (btnEnregistrerClick)
  try
    if FRepository.SaveClient(FClient) then
    begin
      MessageDlg('Client enregistré avec succès', mtInformation, [mbOK], 0);
      ModalResult := mrOk;
    end
    else
...
begin
  ...
  begin
    ...
    AImage.Visible := True;
    // Définir l'icône de validation (coche verte)
  end
  else
  begin
...
begin
  ...
  for I := 0 to AErrors.Count - 1 do
    Message := Message + '• ' + AErrors[I] + sLineBreak;
...
begin  
  // Copier tous les champs de l'interface vers FClient AVANT validation
...
begin  
  if FModified then
  begin
    if MessageDlg('Des modifications ont été effectuées. Voulez-vous vraiment quitter sans enregistrer ?',
      mtConfirmation, [mbYes, mbNo], 0) = mrYes then
...
### 1. Barre de progression pour les opérations longues
...
begin
  ...
  try
    ...
    lblStatus.Caption := 'Import terminé !';
    ShowMessage(Format('%d clients importés avec succès', [Total]));
...
type
  ...
  // Formulaire dédié — possède son propre Timer pour pouvoir se fermer tout seul
  TNotificationForm = class(TForm)
  private
    FTimer: TTimer;
    procedure TimerEvent(Sender: TObject);
  public
    constructor CreateToast(const AMessage: string; AType: TNotificationType);
  end;
...
constructor TNotificationForm.CreateToast(const AMessage: string;
...
begin
  ...
  FTimer.OnTimer := TimerEvent;  // TNotifyEvent : méthode de classe, OK
  FTimer.Enabled := True;
end;

procedure TNotificationForm.TimerEvent(Sender: TObject);  
begin  
  FTimer.Enabled := False;
  Release;  // libère le formulaire en sécurité quand la pile d'événements est vide
end;
...
begin
  Form := TNotificationForm.CreateToast(AMessage, AType);
  Form.Show;  // le formulaire se libère seul via TimerEvent (Release)
end;
...
Utilisation :
```pascal
TNotificationHelper.Show('Client enregistré avec succès', ntSuccess);  
...
uses
  uDMConnection, FireDAC.Comp.Client, Vcl.Graphics;
...
begin
  ...
  try
    ...
    Query.Open;
    lblRevenueMonth.Caption := FormatFloat('#,##0.00 €',
...
### Application de thèmes modernes
...
var  
  StyleName: string;
begin
  // Appliquer un style VCL moderne déjà inclus dans le projet (option « Application
  // Appearance » dans les options du projet).
  TStyleManager.TrySetStyle('Windows11 Modern Dark');

  // Ou charger un style personnalisé. ⚠️ LoadFromFile **enregistre** le style
  // dans le gestionnaire et renvoie le nom sous lequel il a été enregistré,
  // mais ne l'**applique** pas. Il faut explicitement appeler SetStyle après.
  if FileExists('MonStyle.vsf') then
  begin
    StyleName := TStyleManager.LoadFromFile('MonStyle.vsf');
...
> ⚠️ Pour que `FormKeyDown` reçoive les touches même quand un autre contrôle a le focus, n'oubliez pas de positionner `KeyPreview := True` sur la fiche (dans l'inspecteur d'objets ou via le code).

```pascal
procedure TFormClients.FormCreate(Sender: TObject);  
begin  
  KeyPreview := True;  // Indispensable pour intercepter les raccourcis globalement

  // Définir les hints (les raccourcis sont gérés par FormKeyDown)
...
### 2. Mémorisation de la position des fenêtres
...
begin  
  // Définir les TabOrder correctement
  edtNom.TabOrder := 0;
...
Une interface utilisateur évoluée nécessite :
- Une architecture claire et modulaire
- Des composants bien configurés et personnalisés
- Des validations en temps réel
- Un feedback visuel constant
- Une navigation intuitive
- Des raccourcis clavier
- Une cohérence visuelle
- Une attention à l'accessibilité

Les interfaces modernes combinent esthétique et fonctionnalité pour offrir la meilleure expérience utilisateur possible.

Dans le prochain chapitre, nous verrons comment créer des rapports et des tableaux de bord pour exploiter toutes ces données.

⏭️ [Rapports et tableaux de bord](/19-projets-avances/01.4-rapports-tableaux-de-bord.md)
