...
Les rapports et les tableaux de bord sont des outils essentiels pour toute application de gestion. Ils permettent de visualiser, d'analyser et de présenter les données de manière claire et professionnelle. Dans ce chapitre, nous allons apprendre à créer des rapports sophistiqués et des tableaux de bord interactifs avec Delphi.

## Types de rapports

### 1. Rapports simples
- Listes de données (clients, produits, commandes)
- Rapports avec en-têtes et pieds de page
...
begin
  ...
  FooterText := Format('Page %d - Imprimé le %s',
    [APageNumber, FormatDateTime('dd/mm/yyyy à hh:nn', Now)]);
  Printer.Canvas.Font.Size := 8;
...
begin
  ...
  try
    ...
    try
      ...
      SimplePrinter.DrawLine(SimplePrinter.CurrentY);
      SimplePrinter.CurrentY := SimplePrinter.CurrentY + 20;
      ...
      begin
        ...
        if SimplePrinter.CurrentY > (Printer.PageHeight - SimplePrinter.MarginBottom - 100) then
        begin
...
### 1. Création d'un composant d'aperçu
...
procedure TFormPrintPreview.FormDestroy(Sender: TObject);  
begin  
  // FBitmap est créé dans FormCreate : il faut le libérer ici sinon fuite mémoire
  // à chaque ouverture/fermeture de l'aperçu.
  FBitmap.Free;
end;

procedure TFormPrintPreview.LoadFromMetafile(AMetafile: TMetafile);  
begin  
  // Créer un bitmap à partir du métafile
...
begin
  ...
  // Redimensionner l'image d'aperçu
...
begin
  // Si l'utilisateur clique sur « ajuster » alors qu'aucun document n'a encore
  // été chargé via LoadFromMetafile, FBitmap.Width / FBitmap.Height valent 0
  // et la division déclencherait EZeroDivide.
...
FastReport est un générateur de rapports puissant intégré à Delphi.
...
begin
  ...
  try
    ...
    // Choix de l'action.
    // ⚠️ Mauvais pattern : `MessageDlg([mbYes, mbNo, mbCancel])` est très confus pour
    // l'utilisateur final ("Oui = Aperçu" n'a aucun sens visuellement). On utilise
    // TTaskDialog (Vcl.Dialogs, disponible depuis Delphi XE2) avec des command links
    // qui affichent un libellé clair (Caption) et une explication (CommandLinkHint).
    // Note : Buttons.Add renvoie un TTaskDialogBaseButtonItem ; il faut le caster en
    // TTaskDialogButtonItem pour accéder à CommandLinkHint.
    var Choice: Integer;
    var Dialog: TTaskDialog;
    Dialog := TTaskDialog.Create(nil);
    try
      Dialog.Caption := 'Génération du rapport';
      Dialog.Title := 'Liste des clients';
      Dialog.Text := 'Comment souhaitez-vous générer ce rapport ?';
      Dialog.CommonButtons := [tcbCancel];

      with TTaskDialogButtonItem(Dialog.Buttons.Add) do
      begin
        Caption := 'Aperçu à l''écran';
        CommandLinkHint := 'Visualiser le rapport avant de l''imprimer ou de l''exporter';
        ModalResult := 100;
      end;
      with TTaskDialogButtonItem(Dialog.Buttons.Add) do
      begin
        Caption := 'Exporter en PDF';
        CommandLinkHint := 'Enregistrer le rapport au format PDF';
        ModalResult := 101;
      end;
      with TTaskDialogButtonItem(Dialog.Buttons.Add) do
      begin
        Caption := 'Imprimer';
        CommandLinkHint := 'Envoyer directement à l''imprimante par défaut';
        ModalResult := 102;
      end;

      // tfAllowDialogCancellation : autorise la fermeture via Escape ou la croix.
      Dialog.Flags := [tfUseCommandLinks, tfAllowDialogCancellation];
      if Dialog.Execute then
        Choice := Dialog.ModalResult
      else
        Choice := mrCancel;
    finally
      Dialog.Free;
        ...
        begin
          ...
          try
            ...
            begin
              ...
              ShowMessage('Rapport exporté avec succès');
            end;
          finally
            SaveDialog.Free;
          end;
...
### 1. Classe pour générer une facture
...
begin
  ...
  FColumnX2 := PageWidth - 500; // Quantité
...
begin  
  ...
  Inc(FCurrentY, 25);
  Printer.Canvas.TextOut(FMarginLeft, FCurrentY, 'Tél : 01 23 45 67 89');
  Inc(FCurrentY, 25);
...
begin
  ...
  Printer.Canvas.Font.Size := 10;
  Printer.Canvas.Font.Style := [fsBold];
  Printer.Canvas.TextOut(RightX, FCurrentY, 'FACTURÉ À :');
  Inc(FCurrentY, 30);
...
begin  
  ...
  Printer.Canvas.TextOut(FMarginLeft, FCurrentY,
    'Échéance : ' + DateToStr(FFacture.DateEcheance));
  Inc(FCurrentY, 30);
...
begin  
  ...
  Printer.Canvas.TextOut(FColumnX2, FCurrentY, 'Qté');
...
begin
  ...
  Printer.Canvas.TextOut(ValueX, FCurrentY,
    FormatFloat('#,##0.00', FFacture.MontantHT) + ' €');
  Inc(FCurrentY, 30);

  // TVA
  Printer.Canvas.TextOut(LabelX, FCurrentY, 'TVA :');
  Printer.Canvas.TextOut(ValueX, FCurrentY,
    FormatFloat('#,##0.00', FFacture.MontantTVA) + ' €');
  Inc(FCurrentY, 30);
  ...
  Printer.Canvas.TextOut(ValueX, FCurrentY,
    FormatFloat('#,##0.00', FFacture.MontantTTC) + ' €');
...
begin  
  // Créer un aperçu (à implémenter avec TFormPrintPreview)
  Generate;
end;
...
var  
  ...
  Series: TPieSeries;
begin
  StyleChart(AChart, 'Répartition des ventes par catégorie');
...
var  
  ...
  Series: TLineSeries;
begin
  StyleChart(AChart, 'Évolution du chiffre d''affaires');
...
begin
  StyleChart(AChart, 'État des stocks');

  AChart.SeriesList.Clear;

  // Série pour le stock actuel
  ...
  try
    Query.Connection := FConnection;
    // Note : on filtre `stock_minimum > 0` pour éviter une division par zéro
    // dans l'ORDER BY (un produit sans seuil minimum n'a de toute façon pas
    // de raison d'apparaître dans un rapport de stock bas).
    Query.SQL.Text :=
      'SELECT ' +
      '  nom_produit, ' +
      '  quantite_stock, ' +
...
var  
  ...
  Series: TPieSeries;
begin
  StyleChart(AChart, 'Répartition des clients');
...
begin
  ...
  try
    ...
    Query.Open;
    lblCAJourValeur.Caption := FormatFloat('#,##0.00 €',
      Query.FieldByName('total').AsFloat);
    Query.Close;
    ...
    Query.Open;
    lblCAMoisValeur.Caption := FormatFloat('#,##0.00 €',
      Query.FieldByName('total').AsFloat);
    Query.Close;
...
begin
  ...
  try
    ...
    begin
      ...
      // (nécessite une bibliothèque d'export PDF)
...
begin  
  ...
  try
    ...
    on E: Exception do
      raise Exception.Create('Excel n''est pas installé ou n''est pas accessible : ' + E.Message);
  end;
end;
...
begin
  if AQuery.IsEmpty then
    raise Exception.Create('Aucune donnée à exporter');
...
La création de rapports et de tableaux de bord professionnels nécessite :
- Une bonne compréhension des besoins utilisateurs
- Le choix des bons outils (composants natifs, FastReport, TeeChart)
- Une présentation claire et esthétique des données
- Des fonctionnalités d'export vers différents formats
...
Un bon système de reporting améliore considérablement la valeur d'une application de gestion.

⏭️ [Application multi-plateformes avec FMX](/19-projets-avances/02-application-multi-plateformes-avec-fmx.md)
